...
let rec flat (e : TypedExp) (vtab : VarTableA) (vtytab : VarTypeTable) (rtab : RetTable) (k : AVal -> ANorm): ANorm =
    ...
    | ArrayLit (elems, tp) -> 
      ...
      flatl elems vtab vtytab rtab letBase
    ...
    | Plus(e1, e2) ->
      ...
      flat e1 vtab vtytab rtab (fun l ->
          flat e2 vtab vtytab rtab (fun r ->
              let t = newVar "t" 
    ...
    | Length(e, tp) ->
      flat e vtab vtytab rtab (fun arr ->
          let t = newVar "arr"
